//funcion para que detectar si existen valores previos en el storage
let carrito = cargarCarrito();
//inicializo la variable para poder trabajar con la funcion obtenerJSON
let productosJSON = [];
//cargo la variable para que no se pierdan los datos almacenados al actualizar la ventana
  ...
  $("#btn-continuar").on('click', function (e) {
    if (carrito.length == 0){
      ...
      Swal.fire({
        ...
        confirmButtonColor: "#EC7F19"
      })
    }
  });
});

//funcion para el renderizado de los productos
...
function ordenarProductos() {
  ...
  //despues de reordenar hay que volver a renderizar
  $(".card-product").remove();
  renderizarProductos();
}
...
function agregarAlCarrito(productoAgregado) {
  ...
  if (encontrado == undefined) {
    ...
    //posicion del producto y despues incremento su cantidad
...
//funcion para rehacer la tabla cada vez que se actualiza la pagina y eliminar productos del carrito
